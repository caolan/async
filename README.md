...
* `drain_once(callback)` - a function that calls `callback` as soon as the last item in the `queue` has returned from the worker, even if the queue is already idle.
